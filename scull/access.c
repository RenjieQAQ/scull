...
//#include <tools/testing/selftests/rcutorture/formal/srcu-cbmc/src/locks.h>
...
DEFINE_SPINLOCK(scull_u_lock);//static spinlock_t scull_u_lock ;//= SPIN_LOCK_UNLOCKED;
...
DEFINE_SPINLOCK(scull_w_lock);//static spinlock_t scull_w_lock ;//= SPIN_LOCK_UNLOCKED;
...
DEFINE_SPINLOCK(scull_c_lock);//static spinlock_t scull_c_lock;// = SPIN_LOCK_UNLOCKED;
...
static struct scull_dev *scull_c_lookfor_device(dev_t key)
{
	...
	sema_init(&(lptr->device.sem),1);//init_MUTEX(&(lptr->device.sem));
...
static void scull_access_setup (dev_t devno, struct scull_adev_info *devinfo)
{
	...
	dev->qset = scull_qset;
	sema_init(&dev->sem,1);//init_MUTEX(&dev->sem);
